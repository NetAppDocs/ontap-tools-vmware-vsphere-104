...
For a non-HA upgrade, power off the ONTAP tools VM, and for an HA upgrade, power off the ONTAP tools management node before making the following changes to the virtual machine (VM) settings.
...
* In case of a non-HA upgrade, reboot the ONTAP tools VM to reflect the changes. In case of an HA upgrade, reboot the ONTAP tools management node to reflect the changes on the node.
