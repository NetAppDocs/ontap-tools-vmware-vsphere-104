...
[NOTE]
Internal ports require node-to-node communication within the cluster.
...
[NOTE]
For HA deployments using overlay networking, ensure UDP port 8472 is open between all nodes. This port enables pod-to-pod communication across nodes; blocking it will interrupt inter-node networking.
